...
`discuss`（討論模式）／`implement`（實作模式）以分段控制項呈現，
MUST 標示當前模式的可存取名稱。切換 MUST NOT 清空既有對話。
預設為 `implement`。

模式的說明文字 MUST 讓應試者理解差異：討論模式下 AI 以說明回應、不產出可套用的
程式碼；實作模式下 AI 直接產出完整實作。兩者皆不限制 AI 的說明完整性。
