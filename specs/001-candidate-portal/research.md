...
## R-015 AI 依提問意圖回應（原「討論模式與實作模式」，2026-08-05 改寫）

**決策**：不提供模式選擇器。由系統提示要求 AI 依提問的意圖決定是否輸出可套用的
程式碼區塊——要求實作、重構或修正時產出完整實作；詢問概念或取捨時回答該問題。

**原決策與其撤銷理由**：原本提供 `discuss`／`implement` 兩種模式，以分段控制項呈現、
持久化於 `interview_session.collaboration_mode`。它是圍欄時代「輕度引導／深入討論」
的直接沿用——那個年代 AI 不能給答案，需要一個旋鈕調整它給多少方向。憲章 v3.0.0
反轉原則 I 之後，兩個模式僅剩「要不要輸出可套用的區塊」這一項差異，而那件事本來
就該由提問決定。撤銷的三個理由：

1. **它想提供的評分訊號，逐字稿裡已經有了。** 應試者有沒有先想清楚再產程式碼，
   讀對話紀錄就看得出來，不需要一個 flag 轉述。
2. **真實的 AI 工具沒有這種開關。** 本平台評估的是「能否透過 AI 完成實作」，
   介面應接近真實工具；平台自創的旋鈕會讓面試測到「有沒有發現那個按鈕」。
3. **它是選填的，因此不是可靠訊號。** 從未切換過的人不一定較差，可能只是沒注意到。

**前置條件**：撤銷前 MUST 先把實作模式的系統提示改為依意圖回應。原提示寫的是
「先給他，再解釋」，概念問題也會被回以完整實作——討論模式實質上是這個過激提示
的逃生口，直接拿掉會造成體驗降級。

**MUST NOT**：此規則 MUST NOT 被實作為輸出限制層。任何時候應試者要求程式碼，
AI MUST 給出完整版本；差別只在「他有沒有要」（憲章原則 I）。

**驗證限制**：腳本化的假回應驗證不了「依意圖回應」——那是模型行為。硬寫關鍵字
判斷只會製造「測過了」的假象。此項以真實模型人工驗證，並記錄於 quickstart V2。

**同時移除**：`interview_session.collaboration_mode`、`chat_message.collaboration_mode`
（遷移 `002_drop_collaboration_mode.sql`）、`PATCH /api/session/collaboration-mode`、
前端 `ModeToggle` 元件與 store 欄位。協作歷程的留存不受影響——對話內容、
程式碼區塊與 `code_change` 的來源歸屬都在別的欄位。
